...
/** Таблица учёток SUPERADMIN (те же колонки и редактирование, что у «Клиенты»). */
export function SuperadminSuperadminsPage() {
  const { rows, loading, load } = useSuperadminList<SuperadminClientAccountRow>(
    () => superadminApi.listSuperadmins(),
  ...
  const [editRoleDraft, setEditRoleDraft] = useState<'USER' | 'COMPANY' | 'SUPERADMIN'>('SUPERADMIN')
  ...
  function beginEdit(r: SuperadminClientAccountRow) {
    ...
    setEditRoleDraft((r.role as 'USER' | 'COMPANY' | 'SUPERADMIN') || 'SUPERADMIN')
      ...
      >
        <div>
          ...
          <Typography.Paragraph type="secondary" style={{ marginBottom: 0 }}>
            Учётки с ролью SUPERADMIN. Они не отображаются в разделах «Компании» и «Клиенты».
